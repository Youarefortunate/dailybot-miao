...
def get_tenant_access_token():
    ...
    resp = requests.post(url, json={"app_id": config.FEISHU_APP_ID, "app_secret": config.FEISHU_APP_SECRET})
...
def send_to_group(access_token, target_id, summary):
    ...
    card_content = {
        ...
        "header": {"title": {"tag": "plain_text", "content": "📊 飞书任务助手 · 每日进度总结"}, "template": "blue"},
        "elements": [{"tag": "div", "text": {"tag": "lark_md", "content": summary}}],
    }

    payload = {"receive_id": target_id, "msg_type": "interactive", "content": json.dumps(card_content)}
