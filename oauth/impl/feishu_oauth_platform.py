...
class FeishuOATHPlatform(BaseOATHPlatform):
    ...
    async def callback(self, code: str, state: str = None):
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"[{self.OATH_PLATFORM_NAME}] ❌ 授权处理异常: {error_msg}")
    ...
    def send_success_card(self, open_id):
        ...
        try:
            ...
            logger.info(
                f"[{self.OATH_PLATFORM_NAME}] ✨ 已向用户 {open_id} 推送授权成功反馈卡片"
            )
        except Exception as e:
    ...
    def send_failure_card(self, open_id, reason):
        ...
        try:
            ...
            logger.info(
                f"[{self.OATH_PLATFORM_NAME}] ❌ 已向用户 {open_id} 推送授权失败反馈卡片"
            )
        except Exception as e:
    ...
    def send_auth_nudge(self):
        ...
        try:
            ...
            logger.info(f"[{self.OATH_PLATFORM_NAME}] 🚀 已向群聊发送授权引导卡片")
            return True, None
        except Exception as e:
            reason = str(e)
            logger.error(f"[{self.OATH_PLATFORM_NAME}] ❌ 发送引导卡片失败: {reason}")
            return False, reason
