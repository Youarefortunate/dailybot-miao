...
class FeishuTokenStorage(BaseTokenStorage):
    ...
    async def get_token(self, identifier: str = None, **kwargs) -> Optional[str]:
        if not identifier:
            identifier = await self.get_current_open_id()
    ...
    async def get_app_token(
        ...
        return await self._fetch_and_sync_app_token()
        ...
        **kwargs,
        ...
        try:
            if not tenant_access_token:
                tenant_access_token = await self.get_app_token(identifier=identifier)

            if not tenant_access_token:
                return None

            refresh_req = use_request(apis.feishu_user_auth.refresh_user_token)
            data = await refresh_req.fetch(
                {
    ...
    async def save_token(
        ...
        **kwargs,
        ...
        if self.factory:
            await self.factory.set_platform_entry(
    ...
    async def get_current_open_id(self) -> Optional[str]:
        if not self._data:
            return None
    ...
    async def _fetch_and_sync_app_token(self) -> Optional[str]:
        try:
